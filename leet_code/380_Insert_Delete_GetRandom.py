...
# Размышления
# Вставка и удаление делается через сет за 1, но из сета нельзя брать random.
# Потому нужен массив, но из массива уже нельзя будет удалять за 1, так как придётся
# искать элемент, а затем ещё и сдвигать следующие элементы.
# Потому применим два приёма:
# 1) Превратим сет в дикт, и будем хранить в нём индекс.
# Это позволит отыскать элемент за 1.
# 2) Удаление из массива происходит за 1, если удаляем из самого конца,
# так как ничего не придётся сдвигать.

import random

class RandomizedSet:
    def __init__(self):
        self.indices = {}  # Сет для удаления и дублей
        self.values = []  # Список для рандома
    ...
    def remove(self, val: int) -> bool:
        if val in self.indices:
            # Удаление в массиве происходит за 1 только из конца
            # Поэтому поменяем местами последний и удаляемый перед удалением
...
print(obj.getRandom())  # 1,2,3
print(obj.remove(1))  # True
print(obj.getRandom())  # 2,3
